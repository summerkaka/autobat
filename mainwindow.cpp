...
{
    ui->setupUi(this);
    timerId_upadteUi = startTimer(1000);
...
void MainWindow::timerEvent(QTimerEvent *event)
{
    ...
    if (event->timerId() == timerId_upadteUi) {
        // battery 1
        pthread_rwlock_rdlock(&lock_bat1);
        ...
        // battery 2
        pthread_rwlock_rdlock(&lock_bat2);
        ...
        // heater
        pthread_rwlock_rdlock(&lock_heater);
        ...
        pthread_rwlock_unlock(&lock_heater);
...
void MainWindow::HeaterTabWrite(uint8_t item, const void *data)
{
    ...
    case 0x04: // heater on/off
        ...
        break;
    case 0x05: // setpoint
        ui->tableWidget_heater->setItem(4, 0, new QTableWidgetItem(QString::number(*(float *)data, 'f', 4)));
        break;
    case 0x06: // kp
        ui->tableWidget_heater->setItem(5, 0, new QTableWidgetItem(QString::number(*(int16_t *)data)));
        break;
    case 0x07: // ki
        ui->tableWidget_heater->setItem(6, 0, new QTableWidgetItem(QString::number(*(int16_t *)data)));
        break;
    case 0x08: // kd
        ui->tableWidget_heater->setItem(7, 0, new QTableWidgetItem(QString::number(*(int16_t *)data)));
        break;
    default : break;
    }
}
...
void MainWindow::BatteryTabWrite(uint8_t bat_index, uint8_t item, const void *data)
{
    ...
    case 0x00:  // status
        ...
        break;
    case 0x01:  // voltage
//        ui->tableWidget_bat->item(item, bat_index)->setText(QString::number(*(float *)data)); const function cant use setText?
...
void MainWindow::on_radioButton_heaterOn_clicked(bool checked)
{
    ...
    if (SendCommand(CANID_CHARGE, CANID_MB, CMD_WRITE_DIO, data, 5) == 0) {
        ...
        if (radio_heater == kHeaterNoClick) {
            ui->radioButton_heaterOn->setAutoExclusive(false);
            ui->radioButton_heaterOn->setChecked(false);
            ui->radioButton_heaterOn->setAutoExclusive(true);
        }else if (radio_heater == kHeaterOff) {
            ui->radioButton_heaterOff->setChecked(true);
        }
        Pop(-1);
    }
...
void MainWindow::on_radioButton_heaterOff_clicked(bool checked)
{
    ...
    if (SendCommand(CANID_CHARGE, CANID_MB, CMD_WRITE_DIO, data, 5) == 0) {
        ...
        if (radio_heater == kHeaterNoClick) {
            ui->radioButton_heaterOff->setAutoExclusive(false);
            ui->radioButton_heaterOff->setChecked(false);
            ui->radioButton_heaterOff->setAutoExclusive(true);
        }else if (radio_heater == kHeaterOn) {
            ui->radioButton_heaterOn->setChecked(true);
        }
        Pop(-1);
    }
...
//    Heater.setpoint = ui->lineEdit_heaterSP->text().toFloat(&ok);
    ...
    Pop(SendCommand(CANID_CHARGE, CANID_MB, CMD_WR_ZONE, data, 4));
}
...
//    Heater.kp = ui->lineEdit_heaterKp->text().toInt(&ok, 10);
    pid.kp = ui->lineEdit_heaterKp->text().toInt(&ok, 10);
    data[0] = 0x03;
    data[1] = 0x05;
    WriteWordL(&data[2], pid.kp);
    WriteWordL(&data[4], Heater.ki);
    WriteWordL(&data[6], Heater.kd);
    Pop(SendCommand(CANID_CHARGE, CANID_MB, CMD_WR_ZONE, data, 8));
}
